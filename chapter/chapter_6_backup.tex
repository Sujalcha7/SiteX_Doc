...
This section evaluates the performance of the predictive model used in the SiteX application. The production model is the \textbf{Heterogeneous Graph Neural Network (HeteroGAT)}, which was trained via node-level regression to predict the location suitability score. To contextualize its performance, results for the \textbf{XGBoost baseline comparator} are also presented. Both models were evaluated on the same train/validation/test splits with matched preprocessing to ensure a fair comparison.

\subsection{Baseline (XGBoost) --- Feature Importance}
Figure \ref{fig:feature-importance} displays the top 15 features learned by the XGBoost baseline model. The most significant feature is \verb|total_poi_count_km|, followed by \verb|health_ratio| and \verb|lng|. This indicates that the density of Points of Interest (POIs) and proximity to health facilities are critical factors in determining site suitability, consistent with domain knowledge and later reflected in the HeteroGAT graph structure.
...
\subsection{Baseline (XGBoost) --- Learning Curve}
The learning curve in Figure \ref{fig:learning-curve} shows the Root Mean Square Error (RMSE) for both the training and test sets over boosting iterations. The convergence of both curves indicates that the baseline model is learning effectively without significant overfitting.
...
\subsection{Baseline (XGBoost) --- Performance Metrics}
Figure \ref{fig:performance-metrics} compares the XGBoost baseline's performance on the training and test sets using Mean Absolute Error (MAE), Root Mean Square Error (RMSE), and R-squared ($R^2$). High $R^2$ values indicate that the tabular model explains a large portion of the variance in the target variable, establishing a strong baseline for the HeteroGAT to surpass.
...
\subsection{Baseline (XGBoost) --- Actual vs.\ Predicted Scores}
The scatter plots in Figure \ref{fig:actual-vs-predicted} visualize the relationship between the actual POI composite scores and the scores predicted by the XGBoost baseline. The strong linear alignment confirms the baseline model's accuracy as a comparator.
...
\subsection{Baseline (XGBoost) --- SHAP Summary}
To interpret the XGBoost baseline's predictions, SHAP (SHapley Additive exPlanations) values were computed. Figure \ref{fig:shap-summary} shows the impact of top features on the model output. For instance, high values of \verb|total_poi_count_km| (red points) generally lead to a higher impact on the predicted score. These SHAP attributions informed which features were promoted to node attributes in the HeteroGAT graph.
...
\subsection{Baseline (XGBoost) --- Spatial Error Distribution}
Figure \ref{fig:error-map} maps the absolute error of baseline predictions on the test set. This visualization helps identify geographic regions where the tabular model performs poorly, motivating the switch to a graph-based model that can capture spatial context more effectively.
...
\subsection{Baseline (XGBoost) --- Residual Analysis}
Figure \ref{fig:residuals} presents the residuals plotted against predicted values and the distribution of residuals for the baseline model. The residuals appear randomly distributed around zero and approximately normal, satisfying the assumptions of regression analysis and indicating no systematic bias in the baseline.
...
\subsection{Baseline (XGBoost) --- Data Distribution}
Figure \ref{fig:data-dist} shows the distribution of the target variable and the geographic split of training and test data, confirming both sets are representative of the overall dataset and that the target distribution is well-covered.
...
% ------- Production Model: HeteroGAT -------
\subsection{Production Model (HeteroGAT) --- Performance}
\label{subsec:heterogat-perf}

The production \textbf{HeteroGAT} model replaces the tabular XGBoost pipeline with a heterogeneous graph neural network that performs relation-aware attention-based message passing across candidate, POI, competitor, and contextual road nodes.

\paragraph{Evaluation protocol.}
HeteroGAT is evaluated under the same train/validation/test split used for the XGBoost baseline, plus a spatial block-holdout split to test geographic generalization. Metrics reported are MAE, RMSE, $R^2$, mean residual, and standard deviation of residuals.

\paragraph{Expected outcomes.}
\begin{itemize}
    \item \textbf{Lower MAE/RMSE than XGBoost:} The graph-based model captures multi-hop spatial dependencies that are invisible to tabular features, expected to reduce prediction error in dense and mixed-use areas.
    \item \textbf{Improved spatial generalization:} Spatial holdout results are expected to show smaller performance degradation compared to the XGBoost baseline, as the GNN learns transferable neighborhood patterns rather than memorizing tabular feature distributions.
    \item \textbf{Attention-based interpretability:} Relation-level attention weights provide a natural alternative to SHAP for explaining which neighboring POI/competitor types most influence each prediction.
\end{itemize}

\textit{Note: Final HeteroGAT quantitative results (loss curves, metric tables, and comparison plots) will be presented once training is complete. Placeholder sections are provided here for consistency with the evaluation framework described in Chapter~\ref{chap:methodology}.}
