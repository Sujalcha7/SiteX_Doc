...
In the ongoing $8^{th}$ semester, our core focus has shifted towards a major architectural upgrade of our predictive engine. We have transitioned from the tabular XGBoost model to a Heterogeneous Graph Neural Network (HeteroGNN) architecture, specifically implementing a Heterogeneous Graph Attention Network (HeteroGAT). This transition was necessary because traditional tabular models struggle to fully capture the complex, multi-hop spatial relationships and interactions between different entities (e.g., candidate sites, competitors, points of interest, and road networks) in an urban environment. By representing our data as a typed graph, the HeteroGNN can directly learn relation-specific transformations and attention weights, leading to more nuanced and context-aware suitability predictions. Our $8^{th}$ semester work also involves refining the existing architecture, integrating real-world "To-Let" property data, conducting real-world user testing, and delivering the final, full-featured version of the platform.