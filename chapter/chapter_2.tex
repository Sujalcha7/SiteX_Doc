...
\par To overcome the limitations of traditional gravity-based models, researchers have increasingly adopted machine learning techniques for retail site selection. Machine learning models are capable of identifying complex, nonlinear relationships among multiple spatial and demographic variables without requiring explicit assumptions. Smith et al. demonstrated the effectiveness of such approaches by applying tree-based models to predict the sales performance of new commercial stores \citep{smith2022machine}.
...
\par The integration of Geographic Information Systems (GIS) with the Analytical Hierarchy Process (AHP) represents a well-established multi-criteria decision-making approach for site selection \citep{doe2023gis}. The AHP framework decomposes the site selection problem into a hierarchy of criteria, subcriteria, and location alternatives, with stakeholders assigning weights through pairwise comparisons. GIS spatializes these criteria into raster or vector datasets, enabling geographic visualization and overlay analysis. In the context of a Predictive Site Selection System, the GIS-AHP methodology provides complementary insights to machine learning approacheswhile AHP captures expert knowledge and stakeholder preferences, foundational machine learning models (such as our 7th-semester XGBoost baseline) or advanced spatial models (like our current HeteroGNN) learn from historical transaction data and competitive dynamics. Combining both approaches enables a hybrid decision support system that is both principled in treatment of subjective preferences and data-driven in treatment of empirical relationships.

% \section{Graph Neural Networks in Spatial Analysis}
\par Recent advancements in deep learning have introduced Graph Neural Networks (GNNs) as a powerful tool for modeling complex spatial and relational data. Unlike traditional tabular models like XGBoost, which assume independent and identically distributed data, GNNs explicitly model the interactions between different geographic entities by representing them as nodes and edges in a graph. In the context of location analysis and POI recommendation, Heterogeneous GNNs (HeteroGNNs) have shown exceptional promise by being capable of integrating diverse data types—such as candidate sites, existing competitors, and various urban amenities—into a single, unified topological structure. By employing message-passing and attention mechanisms, such as in Heterogeneous Graph Attention Networks (HeteroGAT), these models can effectively capture the multi-hop, non-Euclidean spatial dependencies that govern urban dynamics. This shift from tabular feature engineering to graph-based representation learning forms the core motivation for upgrading our system architecture from the initial XGBoost baseline to a production HeteroGAT model.
...
%     \hline
%     \citet{jones2021spatial} & GIS-based Feature Engineering & Framework for transforming raw spatial data (buffers, density) into ML features. \\
%     \hline
%     \citet{smith2022machine} & Machine Learning (Tree-based) & ML models outperform traditional statistics by capturing non-linear relationships. \\
%     \hline
%     \citet{doe2023gis} & GIS + Analytical Hierarchy Process (AHP) & Integrates expert subjectivity (AHP) with objective spatial data (GIS). \\
%     \hline
