...
\begin{itemize}
	...
	User requests originating from the UI Frontend enter the system through the API handler. Feature computation processes query the relevant data files and return calculated features. These features are then passed to the prediction process, which generates a specific suitability score for the chosen location. The scores and visual analytics are consolidated and returned to the frontend for presentation.
\end{itemize}
