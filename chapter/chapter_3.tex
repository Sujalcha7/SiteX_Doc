...
\section{Software Requirements and Tech Stack}

The software requirements and technology stack for the development and deployment of the predictive site selection system include:
\begin{enumerate}
    ...
    \item \textbf{Machine Learning \& Spatial Analysis:} \textbf{PyTorch}, \textbf{PyTorch Geometric} (for HeteroGNN implementation), Scikit-learn, XGBoost (for baseline modeling), Pandas, Numpy, NetworkX (for graph-based routing), and GeoPandas (for offline spatial analysis).
    \item \textbf{Data Storage:} \textbf{SpatiaLite} (SQLite with spatial extensions) for structured geospatial data (listings, competitors, POIs, demographics), and a serialized \textbf{OSMnx road graph pickle} (\texttt{road\_graph.pkl}) for the road network. Baseline CSV exports are retained for XGBoost ablation only.
...
\begin{enumerate}
    ...
    \item[\textbf{b.}] \textbf{Map Visualization:} Display a color-coded suitability map of the Kathmandu Valley, highlighting high-potential areas tailored to the user's specific business focus.
